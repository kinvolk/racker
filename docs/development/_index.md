...
Since Racker runs locally, its container image default command extracts the Racker payload into `/opt/racker`, creates symbolic links in `/opt/bin` to its exectuables, sets up any systemd services, and thus expects to be run with privileged permissions:
